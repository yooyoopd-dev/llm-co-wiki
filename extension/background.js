...
async function setBadge(text, color, title, clearAfterMs = 0) {
  ...
  if (clearAfterMs > 0) {
    badgeTimer = setTimeout(() => {
      ...
      void chrome.action.setTitle({ title: "LLM-CO-WIKI Clipper" });
    }, clearAfterMs);
  }
}
...
async function clipCurrentPage(commandTab) {
  ...
  try {
    ...
    if (!project) throw new Error("No LLM-CO-WIKI project is available");
    ...
    await setBadge("✓", "#059669", `Saved to ${project.name || "LLM-CO-WIKI"}`, 4000);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.error("[LLM-CO-WIKI Clipper] shortcut failed:", error);
